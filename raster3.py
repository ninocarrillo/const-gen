...
def sortConst(constellation):
	...
	for i in range(size):
		constellation[0][i] = sorted_constellation[0][i]
		constellation[1][i] = sorted_constellation[1][i]


def genQAM1(sorted_constellation, size, step):
	constellation = np.zeros((2,size))
	# Make a constellation with symmetry across axes
	...
	if success:
		...
		for i in range(size):
			...
			if (xrow % 2) == (yrow % 2):
				...
				j_odd += 2
		print(f'A Count: {int(j_even/2)}, B Count: {int((j_odd - 1)/2)}')
	constellation = sorted_constellation
	return success

def genQAM2(sorted_constellation, size, step):
	constellation = np.zeros((2,size))
...
def main():
	...
	if len(sys.argv) < 3:
		print("Not enough arguments. Usage: python3 raster.py <const point count> <interval>")
		sys.exit(1)

	const_size = int(sys.argv[1])
	const_intv = int(sys.argv[2])
	constellation = np.zeros((2,const_size))

	result = genQAM1(constellation, const_size, const_intv)
	if result:
		# create a circle
		n = 360
		circle = np.zeros((2,n))
